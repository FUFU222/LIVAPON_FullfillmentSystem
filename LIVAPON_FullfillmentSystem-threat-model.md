...
The prior critical risk around broad browser-reachable `orders` writes has been mitigated in the repository schema: `orders` now has vendor/admin-scoped read access and admin-only write policy (`OrdersReadable`, `OrdersAdminWrite`). The main residual high-risk themes are operational abuse of internet-exposed internal worker endpoints protected by static bearer secrets, conditional shop-registration abuse in Shopify OAuth/callback flow, and sensitive-token handling in Supabase.
...
- `orders` RLS is enabled and write access is admin-only in `schema.sql` (`OrdersReadable`, `OrdersAdminWrite`).
- Open questions that can change ranking:
- Are table-level `INSERT/UPDATE/DELETE` grants to `anon/authenticated` revoked in your Supabase project? The repo schema now relies on RLS for admin-only writes; production grants should still be audited periodically.
...
- Validation/authorization: policy-dependent; `orders` has vendor/admin-scoped read policy and admin-only write policy; line items/shipments are vendor/admin scoped.
...
| Orders write policy surface | Supabase SQL API/PostgREST | Authenticated/anon DB role -> `orders` | Current schema restricts writes to admin role; keep DDL regression review in place | `schema.sql:417-436` |

## Top abuse paths
1. Cross-tenant order tampering via DB policy regression
1. Attacker signs in as any authenticated user.
2. Attacker uses Supabase JS/REST directly from browser context.
3. If future DDL reintroduces broad write grants/policies, attacker updates `orders.status` or related fields on other vendors’ orders.
...
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| TM-001 | Authenticated low-privilege user | Future RLS/grant regression plus valid session and direct Supabase access | Writes to `orders` outside app authorization path | Cross-tenant/order integrity compromise; fulfillment mistakes | Order/shipment integrity, tenant boundary | App-side auth checks (`lib/auth.ts`); current `OrdersReadable` / `OrdersAdminWrite` policies (`schema.sql`) | Browser direct Supabase client exists, so DB policy regressions remain high-impact | Keep role-scoped write policies; revoke direct table writes where possible; move writes to vetted RPC/Server Actions only; add migration tests asserting denied cross-tenant writes | Alert on `orders` mutations by non-service roles; monitor anomalous `last_updated_source` and bulk status flips | Low (with current schema) | High | medium |
