...
> ステータス: セラー/管理者用 UI と Shopify 連携は稼働。配送登録は LIVAPON 側で即時受付し、Shopify 同期はバックグラウンド再同期で処理する。CSV インポートとセラー自己取消 / 再送 UI は現行運用スコープ外。
...
3. **セラー出荷管理**:
   - `/orders` で検索・フィルタ、ラインアイテム単位で数量と SKU を確認し、同ページ内の発送登録パネルから複数ラインアイテムを選択して追跡番号＋配送会社を登録。送信後は LIVAPON 側で即時に発送済み扱いにし、Shopify 共有は `sync_status` と再同期 worker が担う。
...
5. **Shopify 同期**:
   - `registerShipmentsFromSelections` が `shipments` / `shipment_line_items` を作成し、`sync_status` を `pending` にする。
   - `resyncPendingShipments` / `syncShipmentWithShopify` が FO 情報を取得し Fulfillment API を呼出、成功で `synced`、未生成 FO はリトライスケジュール。
   - `triggerShipmentResyncForShopifyOrder` が FO 完了 Webhook を受け、保留分を即再同期。
   - 同期失敗時は admin 画面で再同期・Fulfillment ID 紐付け・手動対応済み化を行う。セラーが Shopify 管理画面で直接登録する運用は break-glass のみ。
6. **保留機能（現行運用スコープ外）**:
...
| 発送履歴 | 過去発送の参照、同期状態の確認、修正依頼導線。セルフサービスのキャンセル / 再送は現行スコープ外。|
...
- **UX**: 成功/失敗は Toast + インライン Alert。配送登録の主導線ではジョブ進捗 polling を見せず、即時受付と画面 refresh を優先する。
...
- `sync_pending_until` を消化する GitHub Actions worker の監視強化と Slack / Dashboard 通知。
