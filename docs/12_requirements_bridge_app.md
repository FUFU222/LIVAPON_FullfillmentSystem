...
- **Webhook**:
  ...
  - `fulfillment_orders/order_routing_complete`, `fulfillment_orders/hold_released`, `fulfillment_orders/cancellation_request_accepted` → `triggerShipmentResyncForShopifyOrder` が FO メタ再同期と保留中 Shipment の再同期を実行。
...
- **Bulk Shipment API**: `/api/shopify/orders/shipments` がセラーの一括発送登録を受け付け、CSRF / Supabase Auth / セラー所有 line item / 冪等 `requestId` を検証する。Shopify API の完了は待たずに `shipments` を作成し、`202` を返す。
- **Fulfillment 同期**: `resyncPendingShipments` / `syncShipmentWithShopify` が FO 情報を自動取得し、REST Admin API で Fulfillment 作成/追跡更新。未生成 FO は指数バックオフでリトライ予定時刻を保存。
...
2. Supabase → Shopify
   - セラー UI / API で `shipments` が作成されると `sync_status=pending`。CSV は legacy route のため正式導線ではない。
   - GitHub Actions worker または API 後の best-effort resync が FO を取得し Fulfillment API を呼び出し、成功した追跡番号を Shopify に反映。
   - `sync_status` は `pending` → `processing` → `synced` / `error`。
   - 恒久エラーは admin 画面から再同期・手動対応済み化・Fulfillment ID 紐付けで解消する。Shopify 管理画面での直接操作は例外手順。

## 運用ポイント
- **スコープ**: `read_merchant_managed_fulfillment_orders`, `write_merchant_managed_fulfillment_orders`, `read_orders`, `write_orders` を OAuth リクエストに含める。
- **権限の見方**:
  - `shopify.app.toml` / `SHOPIFY_SCOPES` は「このアプリが要求する権限」。
  - `shopify_connections.scopes` は「そのストアが実際に許可した権限」。
  - 実行時は `access_token` で Shopify API を呼び、`scopes` 列は不足権限の警告用途として扱う。
...
## 未完了・今後の検討
- `sync_status='error'` / stale `processing` の監視を Slack / Dashboard へ展開。
