...
1. ブランチ作成 → ドキュメント確認。
2. 実装 → ローカル検証。
3. Codex 作業時は、指示の有無にかかわらず自分が行った変更はコミットまで完了させる（push も依頼があれば即応）。
4. テスト
   ```bash
   npm run lint
   npm run test
...
6. マージ後、必要なら `supabase db push` で環境反映。

> **コミュニケーションの徹底**: 実装やコマンドによる検証など作業に入る前に、必ず簡潔でもよいので「これから何をするか」を共有する。方針が分かりづらいタスクほど事前に確認してから着手すること。
> **デプロイ方針**: 本番デプロイは原則 GitHub への `git push` → Vercel 自動デプロイで統一する。`vercel --prod` など手動デプロイは緊急時のみ使用し、事前共有と事後記録を必須とする。
