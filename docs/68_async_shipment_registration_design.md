...
現行実装は `shipment_import_jobs` に積む構造を持っているが、ジョブ内でローカル発送レコード作成と Shopify 同期を一緒に行うため、注文一覧の「発送済」反映もジョブ完了に引きずられる。この設計では、ローカル受付と Shopify 同期を明確に分離する。
...
新規 migration で `shipments` に以下を追加する。
